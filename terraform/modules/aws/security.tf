...
resource "aws_security_group" "processing_sg" {
  ...
  description = "Allow SSH from admin and access to S3 and DB from processing VM"
...
resource "aws_security_group" "db_sg" {
  ...
  description = "Allow Postgres from processing and presentation VMs"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.processing_sg.id, aws_security_group.presentation_sg.id]
  }
